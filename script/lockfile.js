...
function unlock(file, options, callback) {
    // Code modified by: Jeremy Dela Cruz
    options = {
        fs,
        realpath: true,
        ...options,
    ...
    resolveCanonicalPath(file, options, (err, file) => {
        ...
        if (!lock) {
            // Handle the specific 'ENOTACQUIRED' error
            if(options.showError === true) {
                const error = Object.assign(new Error('Lock is not acquired/owned by you'), { code: 'ENOTACQUIRED' });
                console.error('Unlock error:', error);
            }

            return callback(null); // Returning null to indicate the operation should continue
        }
